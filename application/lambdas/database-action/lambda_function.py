    ...
    while retries < MAX_RETRIES:
        try:
            ...
            try:
                ...
                logger.info("Agent completion: %s", completion.split(":")[1].strip())
                corrected_sql = completion.split(":")[1].strip()
                ...
                logger.info("Agent provided corrected_SQL query: %s", corrected_sql)
                #sql_query = corrected_sql.rstrip(';')  # Update sql_query with corrected version
                sql_query = corrected_sql.split(';')[0]
                logger.info("Agent provided corrected final SQL query: %s", sql_query)
